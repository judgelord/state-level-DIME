...
As .png
```{r bonica-1-does-NOT-cross-0, fig.height = 4, fig.width = 3}
# fig 1 Political Principals All Democrats OR Republicans
# dist_1_x_zero == 0
# sorted by distance_1_gov_leg_med
d %>% 
  filter(dist_1_x_zero == 0) %>% 
  mutate(party = ifelse(dist_1_x_zero == 0 & Governor<0, "All Democrats", "All Republicans")) %>% 
ggplot() +
  geom_point(aes(x = Governor, y = reorder(state, distance_1_gov_leg_med)), shape = "G", color = 1) + 
...
d %>% 
  ...
  scale_x_continuous(breaks = c(-1, 0, 1)) + 
  facet_grid(party ~ year, scales = "free_y", space = "free_y") + 
  theme_minimal() + theme(axis.text.y = element_text(size = 5), title = element_text(size = 7)) 
ggsave("Figs/bonica-2-does-cross-0.pdf", height = 3, width = 3)
```
