...
csp %<>% 
  mutate(state_party = ifelse(Leg_Composition == "R" & Gov_Party == "R", "All Republican", state_party)) %>% 
  mutate(state_party = ifelse(Leg_Composition == "D" & Gov_Party == "D", "All Democrat", state_party)) %>% 
  mutate(state_party = ifelse(Leg_Composition != Gov_Party, "Divided", state_party))
...
[Download DIME 2004 and 2014 sample merged with Divided Government Data](https://raw.githubusercontent.com/judgelord/state-level-DIME/master/data/DIME+divided_government.csv)
...
Alignment among governors (G) and upper (U) and lower (L) chambers of state legislatures in 2004 and 2008:
...
Alignment among governors (G) and upper (U) and lower (L) chambers of state legislatures in 2004 and 2008:
```{r DIME-party-2003-2008-plots, fig.height = 5, fig.width = 3.1, fig.show='hold'}
d %>% 
  filter(year == 2004) %>% 
  DIMEplot()
d %>% 
