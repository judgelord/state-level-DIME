...
```
```{r bonica-5-all-2004, fig.height = 5, fig.width = 3}
...
ggsave("Figs/bonica-5-all-2004.pdf", height = 5, width = 3)
```
```{r bonica-5-all-2008, fig.height = 5, fig.width = 3}
# fig 5 - All kinds Political Principals
# states sorted by distance_1_gov_leg_med
d %>% 
  filter(year == 2008) %>%
  ...
  geom_point(aes(x = Lower, color = Lower), shape = "L") + 
  geom_point(aes(x = Upper, color = Upper), shape = "U") + 
  labs(y = "States by Preference Divergence", x = "Mean Ideology Score", title = "Preference Divergence of Political Principals, 2007", color = "") +
  scale_x_continuous(breaks = c(-1, 0, 1)) + 
  scale_colour_gradient2(low = "blue", mid = "purple", high = "red") +
  facet_grid(party ~ ., scales = "free_y", space = "free_y") + 
  theme_minimal() + 
...
ggsave("Figs/bonica-5-all-2008.pdf", height = 5, width = 3)
```
